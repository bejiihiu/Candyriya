package kz.bejiihiu.candyriya.i18n;

import com.google.common.reflect.TypeToken;
import kz.bejiihiu.candyriya.i18n.conf.ConfigSpec;
...
public class CandyriyaConfig {

    private static CandyriyaConfig instance;

    private final CommentedConfigurationNode node;
    private final ConfigSpec spec;

    public CandyriyaConfig(CommentedConfigurationNode node) throws ObjectMappingException {
        this.node = node;
    ...
    private static void load() throws Exception {
        Path path = Paths.get("Candyriya.conf");
        CommentedConfigurationNode node = HoconConfigurationLoader.builder().setSource(
            () -> new BufferedReader(new InputStreamReader(CandyriyaConfig.class.getResourceAsStream("/META-INF/Candyriya.conf"), StandardCharsets.UTF_8))
        ).build().load();
        ...
        cur.getNode("locale", "current").setValue(CandyriyaLocale.getInstance().getCurrent());
        fillComments(cur, CandyriyaLocale.getInstance());
        instance = new CandyriyaConfig(cur);
        loader.save(cur);
    }

    private static void fillComments(CommentedConfigurationNode node, CandyriyaLocale locale) {
