...
import kz.bejiihiu.candyriya.common.mod.util.remapper.RemappingClassLoader;
...
public abstract class PluginClassLoaderMixin extends URLClassLoader implements RemappingClassLoader, PluginClassLoaderBridge {
    ...
    public void Candyriya$constructor$super(String name, URL[] urls, ClassLoader parent) {
        throw new AbstractMethodError();
    }

    @CreateConstructor
    public void Candyriya$constructor(String name, @NotNull JavaPluginLoader loader, @Nullable ClassLoader parent, @NotNull PluginDescriptionFile description, @NotNull File dataFolder, @NotNull File file, @Nullable ClassLoader libraryLoader) throws IOException, InvalidPluginException {
        Candyriya$constructor$super(name, new URL[]{file.toURI().toURL()}, parent);
    ...
    public ClassLoaderRemapper getRemapper() {
        if (remapper == null) {
            remapper = CandyriyaRemapper.createClassLoaderRemapper(this);
        }
        return remapper;
    }

    @Override
    public CandyriyaRemapConfig getRemapConfig() {
        return CandyriyaRemapConfig.PLUGIN;
    }

    @Override
    public PluginDescriptionFile Candyriya$desc() {
        return description;
    }

    @Override
    public Class<?> Candyriya$loadFromExternal(String name, boolean initialize, boolean checkLibraries) throws ClassNotFoundException {
        return loadClass0(name, initialize, false, checkLibraries);
    }

    @Override
    public SimplePluginManager Candyriya$getPluginManager() {
        return (SimplePluginManager) ((JavaPluginLoaderBridge) (Object) loader).Candyriya$server().getPluginManager();
    }
    ...
    Class<?> loadClass0(String name, boolean resolve, boolean checkGlobal, boolean checkLibraries) throws ClassNotFoundException {
        synchronized (getClassLoadingLock(name)) {
            ...
            if (!CandyriyaConfig.spec().getCompat().isAdventureIsolatedFromML() || !name.startsWith("net.kyori.adventure.")) {
                try {
        ...
        if (checkGlobal) {
            Class<?> result = ((JavaPluginLoaderBridge)(Object) loader).Candyriya$getClassByName(name, resolve, this.description);
            if (result != null) {
                if (result.getClassLoader() instanceof PluginClassLoaderBridge cl) {
                    ...
                    if (provider != this.description && !this.seenIllegalAccess.contains(provider.getName()) && !cl.Candyriya$getPluginManager().isTransitiveDepend(this.description, provider)) {
                        ...
                        if (this.plugin != null) {
                            ...
                            Candyriya$systemLogger().log(Level.WARNING, "[{0}] Loaded class {1} from {2} which is not a depend or softdepend of this plugin.", new Object[]{this.description.getName(), name, provider.getFullName()});
                        }
                    }
                }
    ...
    protected Class<?> findClass(String name) throws ClassNotFoundException {
        ...
        if (result == null) {
            ...
            if (url != null) {
                ...
                try {
                    ...
                    byteSource = () -> {
                        try (InputStream is = connection.getInputStream()) {
                            ...
                            classBytes = CandyriyaRemapper.SWITCH_TABLE_FIXER.apply(classBytes);
                ...
                Product2<byte[], CodeSource> classBytes = this.getRemapper().remapClass(name, byteSource, connection, CandyriyaRemapConfig.PLUGIN);
