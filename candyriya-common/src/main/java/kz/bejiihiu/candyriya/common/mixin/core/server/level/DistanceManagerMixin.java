...
public abstract class DistanceManagerMixin implements DistanceManagerBridge {
    ...
    @Unique
    private Queue<ChunkHolder> Candyriya$scheduleUpdatingQueue = new LinkedList<>();

    @Override
    public void Candyriya$offerUpdate(ChunkHolder holder) {
        Candyriya$scheduleUpdatingQueue.add(holder);
    }

    @Decorate(method = "runAllUpdates", inject = true, at = @At(value = "INVOKE", target = "Ljava/util/Set;isEmpty()Z"))
    private void Candyriya$runQueuedUpdates(ChunkMap map) {
        final var queue = Candyriya$scheduleUpdatingQueue;
    ...
    private Object Candyriya$nullsafeRemovePlayer(Long2ObjectMap<ServerPlayer> instance, long l) throws Throwable {
