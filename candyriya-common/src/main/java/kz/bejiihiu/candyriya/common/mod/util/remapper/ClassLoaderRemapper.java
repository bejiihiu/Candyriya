...
public class ClassLoaderRemapper extends LenientJarRemapper {
    ...
    private static final String REPLACED_NAME = Type.getInternalName(CandyriyaReflectionHandler.class);
    ...
    public ClassLoaderRemapper(JarMapping jarMapping, JarMapping toBukkitMapping, ClassLoader classLoader) {
        ...
        this.jarMapping.setInheritanceMap(CandyriyaRemapper.INSTANCE.inheritanceMap);
    ...
    public Map.Entry<String, String> tryClimb(Map<String, String> map, String owner, String name, int access) {
        ...
        if (mapped == null && (access == -1 || (!Modifier.isPrivate(access) && !Modifier.isStatic(access)))) {
            ...
            if (CandyriyaRemapper.INSTANCE.inheritanceMap.hasParents(owner)) {
                parents = CandyriyaRemapper.INSTANCE.inheritanceMap.getParents(owner);
            } else {
                parents = GlobalClassRepo.inheritanceProvider().getParents(owner);
                CandyriyaRemapper.INSTANCE.inheritanceMap.setParents(owner, parents);
            }
    ...
    public Product2<byte[], CodeSource> remapClass(String className, Callable<byte[]> byteSource, URLConnection connection, CandyriyaRemapConfig config) throws ClassNotFoundException {
        try {
            CandyriyaClassCache.CacheSegment segment = CandyriyaClassCache.instance().makeSegment(connection);
            ...
            if (optional.isPresent()) {
                ...
                if (CandyriyaConfig.spec().getOptimization().isCachePluginClass()) {
                    ClassWriter cw = new ClassWriter(0);
    ...
    private byte[] remapClassFile(ClassReader reader, final ClassRepo repo) {
        ...
        for (PluginTransformer transformer : CandyriyaRemapper.INSTANCE.getTransformerList()) {
    ...
    private Class<?> generateReflectionHandler() {
        try {
            ClassNode node = MixinService.getService().getBytecodeProvider().getClassNode(Type.getInternalName(CandyriyaReflectionHandler.class));
            Preconditions.checkNotNull(node, "node");
            ClassWriter writer = new ClassWriter(0);
            String name = Type.getInternalName(CandyriyaReflectionHandler.class) + "_" + COUNTER.getAndIncrement();
    ...
    private static class NameRemapper extends Remapper {
        ...
        private static final String ORIGIN = Type.getInternalName(CandyriyaReflectionHandler.class);
    ...
    private static class PluginClassWriter extends ClassWriter {
        ...
        private String getSuper(final String typeName) {
            ...
            return CandyriyaRemapper.getNmsMapper().map(node.superName);
        }
    }
    ...
    private static byte[] dump(byte[] bytes) {
        try {
            if (CandyriyaRemapper.DUMP != null) {
                String className = new ClassReader(bytes).getClassName() + ".class";
                int index = className.lastIndexOf('/');
                if (index != -1) {
                    File file = new File(CandyriyaRemapper.DUMP, className.substring(0, index));
                    file.mkdirs();
                    Files.write(file.toPath().resolve(className.substring(index + 1)), bytes);
                } else {
                    Files.write(CandyriyaRemapper.DUMP.toPath().resolve(className), bytes);
                }
            }
        } catch (Exception e) {
