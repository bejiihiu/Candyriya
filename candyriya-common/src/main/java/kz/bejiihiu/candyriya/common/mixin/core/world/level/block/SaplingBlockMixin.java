...
public abstract class SaplingBlockMixin {
    ...
    public void Candyriya$treeGrow(SaplingBlock saplingBlock, IWorld worldIn, BlockPos pos, BlockState state, Random rand) {
        BlockStateListPopulator populator = new CandyriyaBlockPopulator(worldIn.getWorld());
        this.grow(populator, pos, state, rand);
        if (populator.getBlocks().size() > 0) {
            TreeType treeType = CandyriyaCaptures.getTreeType();
