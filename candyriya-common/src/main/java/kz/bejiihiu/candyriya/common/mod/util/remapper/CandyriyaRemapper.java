...
import kz.bejiihiu.candyriya.common.mod.util.remapper.resource.RemapSourceHandler;
...
public class CandyriyaRemapper {

    public static final CandyriyaRemapper INSTANCE;
    public static final File DUMP;
    public static final Function<byte[], byte[]> SWITCH_TABLE_FIXER;

    static {
        CandyriyaI18nLogger.getLogger("Candyriya").info("loading-mapping");
        try {
            INSTANCE = new CandyriyaRemapper();
            String property = System.getProperty("Candyriya.remapper.dump");
            if (property != null) {
            ...
            SWITCH_TABLE_FIXER = (Function<byte[], byte[]>) Class.forName("kz.bejiihiu.candyriya.boot.asm.SwitchTableFixer").getField("INSTANCE").get(null);
        } catch (Exception e) {
    ...
    public CandyriyaRemapper() throws Exception {
        ...
        this.toNmsMapping.loadMappings(
            new BufferedReader(new InputStreamReader(CandyriyaRemapper.class.getResourceAsStream("/bukkit_srg.srg"))),
            null, null, false
        );
        ...
        this.fromMojMapping.loadMappings(
            new BufferedReader(new InputStreamReader(CandyriyaRemapper.class.getResourceAsStream("/bukkit_moj.srg"))),
                null, null, true
        );
        BiMap<String, String> inverseClassMap = HashBiMap.create(toNmsMapping.classes).inverse();
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(CandyriyaRemapper.class.getResourceAsStream("/inheritanceMap.txt")))) {
        ...
        this.transformerList.add(CandyriyaInterfaceInvokerGen.INSTANCE);
        this.transformerList.add(CandyriyaRedirectAdapter.INSTANCE);
        ...
        this.patchers = CandyriyaPluginPatcher.load(this.transformerList);
