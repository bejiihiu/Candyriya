...
import kz.bejiihiu.candyriya.common.mod.util.remapper.patcher.integrated.IntegratedPatcher;
...
public class CandyriyaPluginPatcher implements PluginTransformer {

    private final List<PluginPatcher> list;

    public CandyriyaPluginPatcher(List<PluginPatcher> list) {
        this.list = list;
    }
    ...
    public static List<PluginPatcher> load(List<PluginTransformer> transformerList) {
        ...
        transformerList.add(new CandyriyaPluginPatcher(list));
        return list;
    }
        ...
        try (JarFile jarFile = new JarFile(file)) {
            ...
            if (jarEntry != null) {
                try (InputStream stream = jarFile.getInputStream(jarEntry)) {
                    ...
                    String name = configuration.getString("Candyriya.patcher");
                    if (name != null) {
                        URLClassLoader loader = new URLClassLoader(new URL[]{file.toURI().toURL()}, CandyriyaPluginPatcher.class.getClassLoader());
