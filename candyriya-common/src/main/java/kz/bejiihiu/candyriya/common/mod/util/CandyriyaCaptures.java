...
public class CandyriyaCaptures {
    ...
    public static void captureNextBlockBreakEventAsPrimaryEvent() {
        ...
        // and its context cannot be tracked by Candyriya directly.
    ...
    public static BlockBreakEventContext popPrimaryBlockBreakEvent() {
        if (!blockBreakEventStack.empty()) {
            ...
            if (!unhandledEvents.isEmpty()) {
                // CandyriyaMod.LOGGER.warn("Unhandled secondary block break event");
    ...
    public static void clearBlockBreakEventContexts() {
        if (!blockBreakEventStack.empty()) {
            // CandyriyaMod.LOGGER.warn("Unhandled block break event");
    ...
    public static void captureQuitMessage(String quitMessage) {
        CandyriyaCaptures.quitMessage = quitMessage;
    }
    ...
    public static void capturePlaceEventDirection(Direction direction) {
        CandyriyaCaptures.placeEventDirection = direction;
    }
    ...
    public static void capturePlaceEventHand(InteractionHand hand) {
        CandyriyaCaptures.placeEventHand = hand;
    }
    ...
    public static void captureTreeType(TreeType treeType) {
        CandyriyaCaptures.treeType = treeType;
    }

    public static TreeType getTreeType() {
        try {
            return treeType == null ? CandyriyaConstants.MOD : treeType;
        } finally {
            treeType = null;
        }
    }

    private static transient AbstractContainerMenu Candyriya$capturedContainer;

    public static void captureWorkbenchContainer(AbstractContainerMenu container) {
        Candyriya$capturedContainer = container;
    }

    public static AbstractContainerMenu getWorkbenchContainer() {
        try {
            return Candyriya$capturedContainer;
        } finally {
            Candyriya$capturedContainer = null;
        }
    }
    ...
    private static Stack<CandyriyaDamageContainer> damageContainers = new Stack<>();

    public static void captureDamageContainer(CandyriyaDamageContainer container) {
        damageContainers.push(container);
    }

    public static CandyriyaDamageContainer getDamageContainer() {
     ...
     * 3. SENTINEL
     * @see kz.bejiihiu.candyriya.common.mod.server.event.EntityEventHandler#monitorLivingDrops(LivingEntity, DamageSource, List, boolean)
     */
