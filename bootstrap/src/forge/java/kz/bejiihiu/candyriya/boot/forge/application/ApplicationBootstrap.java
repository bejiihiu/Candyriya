package kz.bejiihiu.candyriya.boot.forge.application;
...
public class ApplicationBootstrap extends ForgeBootstrap {
    ...
    protected void bootstrapMain(String[] args, List<Path[]> classpath) {
        try {
            ...
            var Candyriya = classpath.stream().map(SecureJar::from).filter(it -> it.moduleDataProvider().name().equals("Candyriya.boot")).findAny().orElseThrow();
            var jar = SecureJar.from(it -> new SimpleJarMetadata("Candyriya.launch", "1.0", Set.of("kz.bejiihiu.candyriya.boot.forge.application"), List.of()), Candyriya.getPrimaryPath());
            boot.add(jar);
            ...
            var bootstrap = layer.findModule("Candyriya.launch").orElseThrow();
    ...
    public void moduleMain(String[] args, List<Path[]> classpath) throws Exception {
        ...
        var secure = selectRuntimeModules(classpath).stream().filter(it -> !mergedModules.contains(it.moduleDataProvider().name()) && !it.moduleDataProvider().name().equals("Candyriya.boot"))
        ...
        var Candyriya = classpath.stream().map(SecureJar::from).filter(it -> it.moduleDataProvider().name().equals("Candyriya.boot")).findAny().orElseThrow();
        secure.add(SecureJar.from(it -> new SimpleJarMetadata(Candyriya.name(), Candyriya.moduleDataProvider().descriptor().rawVersion().orElse("1.0"),
            Candyriya.getPackages().stream().filter(p -> !p.equals("kz.bejiihiu.candyriya.boot.forge.application")).collect(Collectors.toSet()), Candyriya.getProviders()), Candyriya.getPrimaryPath()));
        ...
        try {
            ...
            var services = ServiceLoader.load(layer, BootstrapEntryPoint.class).stream().filter(it -> it.type().getName().contains("Candyriya")).toList();
