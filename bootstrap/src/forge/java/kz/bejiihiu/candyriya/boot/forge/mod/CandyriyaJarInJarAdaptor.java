...
public class CandyriyaJarInJarAdaptor implements IDependencyLocator {

    private static final Logger LOGGER = LoggerFactory.getLogger("CandyriyaJiJ");

    private final IDependencyLocator delegate;

    public CandyriyaJarInJarAdaptor(IDependencyLocator delegate) {
    ...
    static void inject() {
        try {
            ...
            var field = ModDiscoverer.class.getDeclaredField("Candyriya$INSTANCE");
            ...
            var newList = locatorList.stream().map(it -> {
                if (it instanceof JarInJarDependencyLocator) {
                    return new CandyriyaJarInJarAdaptor(it);
                } else {
                    return it;
                }
            }).toList();
