...
import kz.bejiihiu.candyriya.i18n.conf.AsyncCatcherSpec;
...
public class AsyncCatcher implements Implementer {
    ...
    public AsyncCatcher() {
        ...
        this.defaultOp = CandyriyaConfig.spec().getAsyncCatcher().getDefaultOp();
        this.dump = CandyriyaConfig.spec().getAsyncCatcher().isDump();
        this.warn = CandyriyaConfig.spec().getAsyncCatcher().isWarn();
    }
    ...
    private void injectCheck(ClassNode node, MethodNode methodNode, String reason) {
        ...
        AsyncCatcherSpec.Operation operation = CandyriyaConfig.spec().getAsyncCatcher().getOverrides().getOrDefault(reason, defaultOp);
        InsnList insnList = new InsnList();
        LabelNode labelNode = new LabelNode(new Label());
        LabelNode labelNode1 = new LabelNode(new Label());
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "io/izzel/Candyriya/common/mod/server/CandyriyaServer", "isPrimaryThread", "()Z"));
        ...
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "io/izzel/Candyriya/common/mod/server/CandyriyaServer", "getMainThreadExecutor", "()Ljava/util/concurrent/Executor;", false));
